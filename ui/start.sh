...
"$LLAMA_BIN" -m "$MODEL_PATH" --host 127.0.0.1 --port 8000 -c 4096 >/tmp/llama-server.log 2>&1 &
BACK_PID=$!

# Don't block on the backend — start the UI immediately.
echo "[start] launching UI on http://localhost:3000"
LLAMA_SERVER="http://127.0.0.1:8000" MODEL="qwen-Q4_K_M" PORT=3000 "$PYTHON" "$REPO/ui/server.py"
